		       ...
		       const mxm_settings_t *prev)
{
	 ...
	 */
	{
		...
		for (fam = 0; fam < MXM_FIR_FAM_COUNT; fam++) {
			...
			if (!dp1 && !dp2)
				continue;
			write_register(card, MXM_REG_FILTER_FAMILY, (u8)fam);
			if (dp1)
				write_register(card, MXM_REG_FILTER_P1,
					       (u8)settings->filter_p1[fam]);
			if (dp2)
				write_register(card, MXM_REG_FILTER_P2,
					       (u8)settings->filter_p2[fam]);
			touched = 1;
		}
		if (touched || !prev || prev->filter_family != settings->filter_family)
			write_register(card, MXM_REG_FILTER_FAMILY,
				       (u8)settings->filter_family);
	}
	return 0;
}
